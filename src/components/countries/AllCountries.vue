...
<template>
  <section>
    ...
    <div v-else-if="countriesStore.error" class="text-center py-10">
      <p class="text-red-500">{{ countriesStore.error }}</p>
    </div>

    <div v-else>
      <div v-if="!countriesStore.filteredCountries.length" class="text-center py-10">
        <p class="text-gray-500 dark:text-gray-300">No countries found matching your filtration</p>
      </div>

      <section v-else class="grid lg:grid-cols-4 md:grid-cols-3 sm:grid-cols-2 grid-cols-1 gap-10">
        <CountryCard
